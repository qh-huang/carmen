...
inline MapConfig::MapConfig(int sizeX, int sizeY, double res, Point offset) { 
  m_sizeX  = sizeX;
  m_sizeY  = sizeY;
  m_res    = res;
  m_offset = offset;
...
inline MapConfig::MapConfig(const MapConfig& src) {
  m_sizeX  = src.m_sizeX;
  m_sizeY  = src.m_sizeY;
  m_res    = src.m_res;
...
inline bool MapConfig::isValid() const {
