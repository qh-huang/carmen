...
#ifdef USE_DOT
static carmen_list_t *people, *trash, *doors;
...
static void print_dots() {
  ...
  for (i = 0; i < people->length; i++)
    printf("P%d ", ((carmen_dot_person_t *)carmen_list_get(people, i))->id);
  for (i = 0; i < trash->length; i++)
    printf("T%d ", ((carmen_dot_trash_t *)carmen_list_get(trash, i))->id);
  for (i = 0; i < doors->length; i++)
    printf("D%d ", ((carmen_dot_door_t *)carmen_list_get(doors, i))->id);
  printf("]");
  fflush(0);
}
*/
...
static void people_handler(carmen_dot_all_people_msg *people_msg)
{
  ...
  navigator_graphics_update_dynamics();
}
...
int initialize_dynamics(void)
{
  ...
  navigator_graphics_initialize_dynamics(people, trash, doors);
