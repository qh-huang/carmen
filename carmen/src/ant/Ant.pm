...
Ant is a library for simple socket communication between software written in both C and Perl. Currently the Perl interface only supports client-side operations.
...
sub send_client_command_core {
    ...
    # read echo
    $response = $self->readn(length($string) + 2, undef);
    if(!defined $response or length($response) < length($string) + 2) {
	print "Error: Could not read from socket.\n" if $self->{verbose};
	return 0;
    }
    return 1;
}
...
Returns the socket connected to the Ant server.  If the connection has broken, this method may return undef.
...
sub connect_to_server {
    ...
    # communicate telnet parameters
    my $data = $self->readn(6, undef);
    if(!defined $data or length($data) != 6) {
	print "Error: received incomplete telnet command from server ".
	    length($data)."of 6 chars.\n" if $self->{verbose};
	return 0;
    }
    my $datalen = $self->writen($data, 6, undef);
    if($datalen != 6) {
	print "Error: could not send telnet response to server.\n"
	    if $self->{verbose};
	return 0;
    }
    print "Relayed telnet commands.\n" if $self->{verbose};
