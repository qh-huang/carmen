...
static carmen_dot_pos_msg *pos_msg_ptr_ext = NULL;
...
void carmen_dot_subscribe_pos_message(carmen_dot_pos_msg *msg,
  ...
  if(subscribe_how == CARMEN_UNSUBSCRIBE) {
    IPC_unsubscribe(CARMEN_DOT_POS_MSG_NAME, 
		    pos_msg_handler);
    return;
  }
  ...
  if (msg)
    ...
    pos_msg_ptr_ext = (carmen_dot_pos_msg *)
      calloc(1, sizeof(carmen_dot_pos_msg));
    carmen_test_alloc(pos_msg_ptr_ext);
  }

  pos_msg_handler_ext = handler;
  err = IPC_subscribe(CARMEN_DOT_POS_MSG_NAME, 
		      pos_msg_handler, NULL);

  if(subscribe_how == CARMEN_SUBSCRIBE_LATEST)
    IPC_setMsgQueueLength(CARMEN_DOT_POS_MSG_NAME, 1);
  else
    IPC_setMsgQueueLength(CARMEN_DOT_POS_MSG_NAME, 100);

  carmen_test_ipc(err, "Could not subscribe", CARMEN_DOT_POS_MSG_NAME);
}

void carmen_dot_start() {

  IPC_RETURN_TYPE err;
  static carmen_dot_start_msg msg;

  err = IPC_defineMsg(CARMEN_DOT_START_MSG_NAME, IPC_VARIABLE_LENGTH, 
		      CARMEN_DOT_START_MSG_FMT);
  carmen_test_ipc_exit(err, "Could not define message", 
		       CARMEN_DOT_START_MSG_NAME);
  
  strcpy(msg.host, carmen_get_tenchar_host_name());
  msg.timestamp = carmen_get_time_ms();
  err = IPC_publishData(CARMEN_DOT_START_MSG_NAME, &msg);
